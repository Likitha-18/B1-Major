...
log = pd.read_csv("datasetlog/Datasetinfectedhealthy.csv")

log = log.tail(1)
X_ul = log.drop(['imgid','fortnum','label'], axis=1)
...
for n in range(4):
	...
	pred = svclassifier.predict(Xi_test)
	print(accuracy_score(yi_test,pred))
'''
...
for n in range(4):
	...
	pred =classifier.predict(Xi_test)
	print(accuracy_score(yi_test,pred))
	#Sum = Sum + pred
	#print(pred)
